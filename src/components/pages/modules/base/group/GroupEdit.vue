  ...
  methods: {
   ...
   async ModelClick(e, module_index, model_index){
      ...
      await  this.user_permissions[module_index]['models'][model_index]['permissions'].forEach( element => {
          if(!this.form_data.permissions.includes(element.id)) {
    ...
    getGroupsEditData: function () {
      axios
        ...
        .then((response) => {
          ...
          this.form_data.permissions = response.data.permissions.map(
    ...
    submitUserForm: function () {
      axios
        .put("groups/" + this.$route.params.id + "/", this.form_data)
        .then(() => {
          Swal.fire({
            icon: "success",
